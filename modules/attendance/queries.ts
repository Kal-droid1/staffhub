...
export async function updateSettings(cutoffTime: string, officeLatitude?: number, officeLongitude?: number, allowedRadiusMeters?: number) {
  ...
  const data: Record<string, unknown> = { cutoffTime };
  if (officeLatitude !== undefined) data.officeLatitude = officeLatitude;
  if (officeLongitude !== undefined) data.officeLongitude = officeLongitude;
  if (allowedRadiusMeters !== undefined) data.allowedRadiusMeters = allowedRadiusMeters;

  return prisma.settings.upsert({
    where: { id: "singleton" },
    update: data,
    create: { id: "singleton", cutoffTime, officeLatitude: officeLatitude ?? null, officeLongitude: officeLongitude ?? null, allowedRadiusMeters: allowedRadiusMeters ?? 200 },
  });
}
